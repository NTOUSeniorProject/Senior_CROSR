...
if __name__ == '__main__':
    ...
    weight_path = 'checkpoints_20260601_2301/best_val.pth'
    ...
    # ✅ 額外紀錄「已知類別」的真實類別與預測類別
    # 用來產出每一個動作分類的詳細 Diffusion / Confusion Matrix
    y_true_known_cls = []
    y_pred_known_cls = []
    ...
    with torch.no_grad():
        for inputs, labels, is_unknown in val_loader:
            ...
            # 分數融合 (700.0 為權重係數，可根據診斷結果微調)[cite: 9]
            combined_scores = dist_scores + (700.0 * mse_scores)
            
            y_scores_combined.extend(combined_scores.cpu().numpy())
    ...
    plt.title(f'開集辨識混淆矩陣 (Threshold = {optimal_threshold:.2f})')
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.tight_layout()
    plt.savefig('ntu_chart_3_confusion_matrix.png', dpi=300)
    plt.close()

    # 圖表 4：詳細各分類 Diffusion / Confusion Matrix
    # 這張圖只看「已知動作」內部的分類結果：
    # 橫軸：模型預測成哪個動作
    # 縱軸：實際是哪個動作
    # 顏色越深代表比例越高，可以觀察哪些動作容易互相混淆
    if len(y_true_known_cls) > 0:
        class_labels = [f'A{action_id:03d}' for action_id in my_known_actions]

        cm_cls = confusion_matrix(
            y_true_known_cls,
            y_pred_known_cls,
            labels=list(range(num_classes))
        )

        # 轉成 row-normalized percentage
        # 讓每一列加總約為 100%，比較容易看出每個實際類別被分到哪裡
        cm_cls_percent = cm_cls.astype(float) / (cm_cls.sum(axis=1, keepdims=True) + 1e-6) * 100

        # 類別很多時，圖要放大，不然標籤會擠在一起
        fig_size = max(12, num_classes * 0.45)
        ...
        plt.title('各已知動作分類 Diffusion Matrix / Confusion Matrix (%)')
        plt.xlabel('Predicted Action')
        plt.ylabel('True Action')
        plt.xticks(rotation=90)
        plt.yticks(rotation=0)
        plt.tight_layout()
        plt.savefig('ntu_chart_4_class_diffusion_matrix.png', dpi=300)
        plt.close()

        # 另外輸出原始數量矩陣，方便放報告或除錯
        np.savetxt(
            'ntu_chart_4_class_diffusion_matrix_counts.csv',
            cm_cls,
            delimiter=',',
            fmt='%d'
        )
    ...
    print("\n📸 進階效能評估報告已生成：")
    print("   1. ntu_chart_1_distribution.png")
    print("   2. ntu_chart_2_pr_curve.png")
    print("   3. ntu_chart_3_confusion_matrix.png")
    print("   4. ntu_chart_4_class_diffusion_matrix.png")
    print("   5. ntu_chart_4_class_diffusion_matrix_counts.csv")
